...
newSandboxObject:any = [];
  ...
  createDynamicForm(fields: any) {
    ...
    for(let i = 0 ; i < fields.length; i++) {
          formData[fields[i].COMPONENT_TYPE] =
          fields[i].isrequired ? 
          new FormControl('', Validators.required)
            : new FormControl('');
         // added logic for tags
        if(fields[i].COMPONENT_TYPE === 'tags') {
           fields[i].chips.forEach((chip:any) => {
             chips.push(chip.chipName);
           });
          formData[fields[i].COMPONENT_TYPE].value = chips;
        }
...
onChangeDropdown(event:any) {
 console.log(event);
  ...
  ngOnInit(): void {
     ...
     this.newSandboxObject = this.commonService.getDynamicFilter().ATTRIBUTES;
    // console.log(this.newSandboxObject);
    
   // used to creater form dynamically
     this.globalDATA = this.commonService.getDynamicFilter().DATA;
    //console.log("this.globalDATA");
     ...
     /*
     this.commonService.getDynamicFilter().subscribe(data => this.newSandboxObject = data.ATTRIBUTES);
     this.commonService.getDynamicFilter().subscribe(serverData => this.globalDATA = serverData.DATA);
     */
     /*
